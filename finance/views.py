...
class IndexView(LoginRequiredMixin,View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(request, "収支投稿成功しました")
            form.save()
        else:
            messages.error(request, "収支投稿失敗しました")
...
class CategoryView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            messages.success(request, "カテゴリ投稿成功しました")
            form.save()
        else:
            
            messages.error(request, "カテゴリ投稿失敗しました" )
...
class CategoryEditView(LoginRequiredMixin,View):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if formset.is_valid():
            messages.success(request, "カテゴリ編集成功しました")
            formset.save()
        else:
            print(formset.errors)
            messages.error(request, "カテゴリ編集失敗しました")
...
class BalanceDeleteView(LoginRequiredMixin,View):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if balance.user.id != request.user.id:
            messages.success(request, "収支の削除に失敗しました")
            return redirect("finance:index")

        balance.delete()
        messages.success(request, "収支の削除に成功しました")
